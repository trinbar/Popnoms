# Secrets for Eventbrite app
# Find these at: https://www.eventbrite.ca/account-settings/apps
# This file should never go into github!

export EVENTBRITE_KEY="SINIF5SUACTUMOR3XLJOUVQW7PODZLR6DD745U334ZBD5DNIJG"
export EVENTBRITE_SECRET="JKW3IQWTLJOTH7T2FS"
export EVENTBRITE_TOKEN="JPNZQAGVM7G4FMX2RN5A"

# Secrets for Yelp app
# Find these at: https://www.yelp.com/developers/v3/manage_app?
# This file should never go into github!

# export YELP_CLIENT_ID = "aQV3ukS8x-r40HNiyUVJ4Q"
# export YELP_API_KEY = "KaTy1eLn4nyfol6NNi5Ru9XPeJwFjNPgiuHyevu_hhYzqAVmhiWgiWstWmcRGcZYfKoGoywN0eXiL0V-BT00T_12F6zXjNJwglQmJz33rZzUgcM_jnSGeAlKnyjbXHYx"